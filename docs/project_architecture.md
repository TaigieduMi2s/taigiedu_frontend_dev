...
- **台語文化（test） (`/culture-test`)**: `cultureTestPage/CultureTestPage.jsx`，影音資料庫的分類瀏覽頁。
  ...
  - **篩選與呈現整套比照「媒體與社群資源」**（`socialmediaPage/`）：
    - 頁首白底橫幅：**一體化搜尋列（圓角 10px 對齊全站樣式）**，版面參考國家文化記憶庫的搜尋頁（2026-09）：
      預設為「類型下拉（全部）｜關鍵字｜搜尋鈕」；**選了影音或文本後，搜尋列才多出第二個下拉「分類」**，
      變成「類型｜分類｜關鍵字」。分類下拉本身仍是第一層 + 第二層飛出子選單，可跨第一層複選
    - 類型下拉共用 `components/CustomSelect`，外觀（去框、半邊圓角）只在 `CultureTestPage.css` 以 `.ctp-type-select` 覆寫
  ...
  - **影音＋文本混合（2026-09，暫定）**：台語文化要同時收影音與文本，目前先做「混合」方案——
    「全部」時兩種類型一起呈現，用搜尋列的類型下拉切換（`?type=video|text`，全部不寫入）。
    **兩種類型的呈現方式不同**：影音維持「圖片 + 主標」卡片；文本**比照主頁搜尋的結果列**（`mainSearchPage/SearchResults`），
    只在最上面多一行標題：標題 → 作者標籤（主頁搜尋這格放資料來源）+ 外連圖示 → 摘要（最多三行），整列點擊另開新分頁，
    關鍵字在標題／作者／摘要都以紅字標示（同主頁搜尋）。
    樣式直接 import `mainSearchPage/SearchResults.css` 沿用 `sr-*` class，本頁只補標題與關鍵字標示；
    ⚠️ 改動 `SearchResults.css` 會同時影響主頁搜尋、議題融入與本頁的文本結果。
    **「全部」不分分類、只做預覽**：上方「推薦影音」（前 4 筆）、下方「推薦文本」（前 5 筆），
    兩區都有「共 N 筆」與「查看全部」（＝切到該類型並保留關鍵字）；**「全部」不分頁**，完整列表交給單一類型。
    選了影音或文本時才依第一層分區（預覽：影音一列 4 筆、文本 5 筆），有篩選或搜尋時攤平成完整列表 + 分頁。
    文本的關鍵字搜尋會多比對 `author`／`summary`。
    **切換類型會清空分類**（保留關鍵字、回到第 1 頁），因為兩種類型的分類不一定相同；
    「全部」沒有分類下拉，網址上殘留的 `cat`／`sub` 會被忽略。目前兩種類型暫時共用同一套分類。
    ⚠️ 混合的前提是文本資料表的分類與影音「差不多」，**尚未比對確認**；若分類完全不同，需改成先選類型、各自篩選。
    ⚠️ 子計畫一另外希望影音**不要用卡片**、改用一次可顯示更多筆的呈現方式，尚未處理。
    文本假資料由 `buildMockTextItems()` 產生（id 為 `text-N`），**只給前台、不進 `MOCK_ITEMS`**，所以後台看不到也改不到。
...
> - `?type=video`：資料類型（目前只有台語文化使用，允許值由各頁傳入 `parseType()`；「全部」不寫入）
>

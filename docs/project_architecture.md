...
- **台語文化（test） (`/culture-test`)**: `cultureTestPage/CultureTestPage.jsx`，影音資料庫的分類瀏覽頁。
  ...
  - **影音＋文本混合（2026-09，暫定）**：台語文化要同時收影音與文本，目前先做「混合」方案——
    ...
    **兩種類型的呈現方式不同**：影音維持「圖片 + 主標」卡片；文本是**搜尋引擎式結果列**
    （標題 → `作者：OOO` → `日期 · 摘要`，摘要最多兩行），關鍵字在標題以主色、在作者與摘要以粗體標示，
    點標題另開新分頁。
    **「全部」不分分類**：最上面一列「推薦影音」（符合條件的前 4 筆 + 共 N 筆 + 「查看全部」＝切到影音並保留關鍵字），
    下面全部是文本結果列；分頁（每頁 20 筆）只算文本，推薦影音只在第 1 頁出現。
    選了影音或文本時才依第一層分區（預覽：影音一列 4 筆、文本 3 筆），有篩選或搜尋時攤平成完整列表 + 分頁。
